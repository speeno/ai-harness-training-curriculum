# Skill: orchestrator-voucher-monthly-close

## 1. Purpose

`voucher-monthly-close` lab의 3일·1주 과정을 조율합니다. 입력 확인, Skill 실행, Smoke 검증, 파일럿 설계, Runbook·Roadmap 작성을 순서대로 연결합니다.

## 2. Workflow

1. `docs/business-scenario.md`와 `docs/industry-constraints.md`를 읽습니다.
2. `workspace/in/` 입력을 확인합니다.
3. `skills/voucher-monthly-close/SKILL.md`로 `workspace/out/voucher-monthly-close-draft.md`를 만듭니다.
4. `tests/smoke/voucher-monthly-close-smoke.md`로 결과를 검증합니다.
...
정상 흐름: 모든 입력 샘플을 읽고 `voucher-monthly-close-draft.md`를 생성한 뒤 Smoke 체크리스트를 통과합니다.
